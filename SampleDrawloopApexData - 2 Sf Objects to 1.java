...
global class SampleDrawloopApexData2SFObjectsTo1 implements Loop.IApexDataSource {
    ...
    private string opportunityProductObjectName = 'ApexOpportunityProduct',
        productFieldPrefix = 'ApexProduct';
    ...
    private List<Loop.ExternalData.FieldInfo> getSObjectFields(Map<String, Schema.SObjectField> fieldMap) {
        return getSObjectFields(fieldMap, '');
    }
    private List<Loop.ExternalData.FieldInfo> getSObjectFields(Map<String, Schema.SObjectField> fieldMap, string fieldPrefix) {
        ...
        for (string fieldKey : fieldMap.keySet()) {
            ...
            Loop.ExternalData.FieldInfo fieldInfo = new Loop.ExternalData.FieldInfo(fieldPrefix + fieldKey, fieldDescribe.getType());
    ...
    public List<Loop.ExternalData.DataObject> describeObjects(List<string> objectNames) {
        ...
        for (string objectName : objectNames) {
            ...
            if (objectName == opportunityProductObjectName) {
                // Describe the fields for this object.
                
                fields.addAll(getSObjectFields(Schema.SObjectType.Opportunity.Fields.getMap()));
                fields.addAll(getSObjectFields(Schema.SObjectType.Product2.Fields.getMap(), productFieldPrefix));
            }
    ...
    public Loop.ExternalData.QueryResultSet query(Loop.ExternalData.QueryRequestInfo requestInfo) {
        ...
        for (string objectName : requestInfo.GetObjectNames()) {
            ...
            if (objectName == opportunityProductObjectName) {
                ...
                // Get list of fields from the Opportunity and Product objects
                Map<String, Schema.SObjectField> fieldMap = Schema.SObjectType.Opportunity.Fields.getMap();
                for (string fieldKey : fieldMap.keySet()) {
                    fields.add(fieldKey);
                    queryFields.add(fieldKey);
                }
                
                fieldMap = Schema.SObjectType.Product2.Fields.getMap();
                for (string fieldKey : fieldMap.keySet()) {
                    fields.add(productFieldPrefix + fieldKey);
                    queryFields.add('Product2.' + fieldKey);
                }
                ...
                string query = string.format(
                    ...
                    new List<string>{ string.join(queryFields, ',') }
                );
